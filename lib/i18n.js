...
(function () {
  ...
  const MESSAGES = {
    it: {
      ...
      "cs.log.keepAliveOn": "Keep-alive audio attivo: scansione continua anche con tab in background.",
      "cs.log.keepAliveFail": "Keep-alive audio non disponibile ({err}). Tab potrebbe rallentare in background.",
      "cs.log.visibilityHidden": "Tab in background: continuo lo scraping (keep-alive attivo).",
      "cs.log.visibilityVisible": "Tab tornata in primo piano — recovery kick.",
      "cs.err.scanInProgress": "Scansione già in corso",

      // ── v0.7.0 — Steroids Edition logs ─────────────────────────────────
      "cs.log.observerOn": "Observer attivo: card detection in tempo reale (no polling).",
      "cs.log.observerFail": "IntersectionObserver non disponibile ({err}) — fallback polling.",
      "cs.log.selectorWin": "Selettore card: strategia {strategy} → {n} card.",
      "cs.log.paceFaster": "Pace adattivo: scansione veloce (-30%) — {n} nuovi ad/ciclo.",
      "cs.log.paceSlower": "Pace adattivo: rallento (+50%) — nessun nuovo ad.",
      "cs.log.idleMicroPause": "Micro-pausa anti-detection ({s}s) — simulo lettura utente.",
      "cs.log.keepAliveWebRTC": "Keep-alive WebRTC attivo (fallback audio).",
      "cs.log.keepAliveWakeLock": "Keep-alive WakeLock screen attivo.",
      "cs.log.queueModeOn": "Modalità queue: rete instabile, accumulo {n} item in coda.",
      "cs.log.queueModeOff": "Rete tornata: flush rapido di {n} item in coda.",
      "cs.log.flushRetry": "Flush retry {n}/3 dopo {ms}ms (err: {err}).",
      "cs.log.flushAuthAbort": "Errore auth ({status}) — annullo scansione.",
      "cs.log.resumeFound": "Trovato scan in sospeso ({n} ad, {age}s fa) — riprendo.",
      "cs.log.resumeRejected": "Scan in sospeso scartato dal background — riparto pulito.",
      "cs.log.parallelFlush": "Flush parallelo: {n} batch in volo.",
      "cs.log.seeMoreClicked": "Cliccato bottone paginazione \"{text}\" — FB carica altri risultati.",

      // v0.7.0 overlay additions
      "overlay.speedLabel": "Speed",
      "overlay.speedUnit": "ad/min",
      "overlay.keepAliveAudio": "Audio attivo",
      "overlay.keepAliveWebRTC": "WebRTC attivo",
      "overlay.keepAliveWakeLock": "WakeLock attivo",
      "overlay.keepAliveNone": "Nessun keep-alive",
      "overlay.queueMode": "Rete lenta — buffering {n} item",
      "overlay.btnResume": "Riprendi scansione",
    },
    ...
    en: {
      ...
      "cs.log.keepAliveOn": "Keep-alive audio active: scan keeps running with tab in background.",
      "cs.log.keepAliveFail": "Keep-alive audio unavailable ({err}). Tab may slow down in background.",
      "cs.log.visibilityHidden": "Tab in background: scrape continues (keep-alive active).",
      "cs.log.visibilityVisible": "Tab back in foreground — recovery kick.",
      "cs.err.scanInProgress": "Scan already running",

      // ── v0.7.0 — Steroids Edition logs ─────────────────────────────────
      "cs.log.observerOn": "Observer on: real-time card detection (no polling).",
      "cs.log.observerFail": "IntersectionObserver unavailable ({err}) — polling fallback.",
      "cs.log.selectorWin": "Card selector: strategy {strategy} → {n} cards.",
      "cs.log.paceFaster": "Adaptive pace: faster (-30%) — {n} new ads/cycle.",
      "cs.log.paceSlower": "Adaptive pace: slowing down (+50%) — no new ads.",
      "cs.log.idleMicroPause": "Anti-detection micro-pause ({s}s) — simulating user read.",
      "cs.log.keepAliveWebRTC": "Keep-alive WebRTC active (audio fallback).",
      "cs.log.keepAliveWakeLock": "Keep-alive WakeLock screen active.",
      "cs.log.queueModeOn": "Queue mode: flaky network, buffering {n} items.",
      "cs.log.queueModeOff": "Network back: fast-flushing {n} buffered items.",
      "cs.log.flushRetry": "Flush retry {n}/3 after {ms}ms (err: {err}).",
      "cs.log.flushAuthAbort": "Auth error ({status}) — aborting scan.",
      "cs.log.resumeFound": "Stale scan found ({n} ads, {age}s ago) — resuming.",
      "cs.log.resumeRejected": "Stale scan rejected by background — starting fresh.",
      "cs.log.parallelFlush": "Parallel flush: {n} batches in flight.",
      "cs.log.seeMoreClicked": "Clicked pagination button \"{text}\" — FB loads more results.",

      // v0.7.0 overlay additions
      "overlay.speedLabel": "Speed",
      "overlay.speedUnit": "ads/min",
      "overlay.keepAliveAudio": "Audio on",
      "overlay.keepAliveWebRTC": "WebRTC on",
      "overlay.keepAliveWakeLock": "WakeLock on",
      "overlay.keepAliveNone": "No keep-alive",
      "overlay.queueMode": "Network slow — buffering {n} items",
      "overlay.btnResume": "Resume scan",
    },
  };
